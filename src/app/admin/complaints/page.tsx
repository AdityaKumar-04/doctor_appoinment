...
const STATUS_CFG: Record<TicketStatus, { label: string; icon: React.ReactNode; cls: string }> = {
  open:        { label: "Pending",       icon: <AlertTriangle size={11} />, cls: "bg-amber-500/10 text-amber-400 border-amber-500/20" },
  in_progress: { label: "Investigating", icon: <Clock size={11} />,         cls: "bg-sky-500/10 text-sky-400 border-sky-500/20" },
...
export default function AdminComplaintsPage() {
  ...
  const qs = new URLSearchParams({ type: "complaint" });
  ...
  const investigating = allTickets.filter((t) => t.status === "in_progress").length;
  ...
  const sendReply = async () => {
    ...
    try {
      ...
      setReply("");
      // Optimistically mark as investigating if currently pending
      if (selected.status === "open") {
        setSelected((p) => p ? { ...p, status: "in_progress" } : p);
      }
      await mutateThread();
      mutate();
    } catch (err: unknown) {
    ...
    <div className="page-enter space-y-8 text-left">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-1.5">
          <p className="text-rose-400 text-[11px] font-black uppercase tracking-[0.2em]">Admin · Complaints</p>
          <h1 className="text-4xl font-black text-white tracking-tighter">
            Complaint <span className="gradient-text">System</span>
          </h1>
          <p className="text-slate-500 font-bold text-sm uppercase tracking-wider">
            {pending} pending · {investigating} investigating · {resolved} closed
          </p>
        </div>
        {pending > 0 && (
          <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-rose-500/10 border border-rose-500/20">
            ...
            <span className="text-[10px] font-black text-rose-400 uppercase tracking-widest">{pending} Unreviewed</span>
          </div>
        )}
      </header>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "Pending",       value: pending,       cls: "bg-amber-500/10 border-amber-500/20 text-amber-400" },
          { label: "Investigating", value: investigating,  cls: "bg-sky-500/10 border-sky-500/20 text-sky-400" },
          { label: "Closed",        value: resolved,       cls: "bg-white/[0.03] border-white/10 text-slate-400" },
        ].map((s) => (
        ...
        <div className="lg:col-span-2 space-y-3">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {(["all", "open", "in_progress", "resolved"] as const).map((s) => (
              ...
              >
                {s === "all" ? "All" : s === "in_progress" ? "Investigating" : STATUS_CFG[s as TicketStatus]?.label || s}
              </button>
            ))}
          </div>
            ...
            {(["all", "patient", "doctor", "clinic"] as const).map((r) => (
              <button
                ...
                className={`inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest border transition-all ${
                  roleFilter === r
                    ? r === "all" ? "bg-white/[0.08] border-white/20 text-white" : (ROLE_COLOR[r] || "bg-white/[0.08] border-white/20 text-white")
                    : "bg-white/[0.02] border-white/5 text-slate-600 hover:text-slate-400"
                }`}
              >
                {r !== "all" && ROLE_ICON[r]}
          ...
          {loading ? (
            ...
            <div className="text-center py-16 opacity-40">
              ...
              <p className="text-xs font-bold uppercase tracking-widest">No complaints found</p>
            </div>
          ) : (
            allTickets.map((t) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-4 rounded-2xl border transition-all hover:border-rose-500/30 ${
                    selected?.id === t.id ? "bg-rose-500/5 border-rose-500/30" : "bg-white/[0.02] border-white/5"
                  }`}
                >
                  ...
                  <div className="flex gap-1.5 flex-wrap">
                    ...
                    <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest border ${PRIORITY_CFG[t.priority] || ""}`}>
                      <Flag size={8} className="inline mr-0.5" />{t.priority}
                    </span>
                  </div>
                </button>
              );
        ...
        <div className="lg:col-span-3">
          {selected ? (
            <Card glass className="p-6 space-y-5 sticky top-6">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">
                    #{selected.id.split("-").pop()?.toUpperCase()} · Complaint
                  </p>
                  <h2 className="text-lg font-black text-white leading-snug">{selected.subject}</h2>
                </div>
                <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest border shrink-0 ${STATUS_CFG[selected.status]?.cls || STATUS_CFG.open.cls}`}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 rounded-xl bg-white/[0.02] border border-white/5">
                  <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest mb-1.5">Filed By</p>
                  <p className="text-sm font-bold text-white">
                    {selected.users?.first_name || ""} {selected.users?.last_name || "User"}
                  </p>
                  <p className="text-[11px] text-slate-500">{selected.users?.email || "—"}</p>
                </div>
                <div className="p-3 rounded-xl bg-white/[0.02] border border-white/5">
                  <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest mb-1.5">Category</p>
                  <div className="flex flex-wrap gap-1.5">
                    <span className={`text-[9px] font-black uppercase tracking-widest border px-2 py-0.5 rounded ${ROLE_COLOR[selected.role] || "text-slate-400 border-white/10 bg-white/5"}`}>
                      {selected.role}
                    </span>
                    <span className={`text-[9px] font-black uppercase tracking-widest border px-2 py-0.5 rounded ${PRIORITY_CFG[selected.priority] || ""}`}>
              ...
              <div className="p-4 rounded-xl bg-white/[0.02] border border-white/5">
                <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-2">Complaint</p>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => mutateThread()} disabled={threadLoading} className="text-slate-700 hover:text-slate-400">
                ...
                {messages.length > 0 && (
                  <div className="max-h-56 overflow-y-auto space-y-2 pr-1">
                    {messages.map((msg) => {
                       ...
                       * - sender_role === "admin" → RIGHT (violet)
                       * - patient/doctor/clinic → LEFT
                       */
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <Button
                      ...
                      disabled={selected.status === "in_progress" || updating}
                      variant="secondary"
                      className="h-9 rounded-xl text-sky-400 border-sky-500/20 hover:bg-sky-500 hover:text-white font-black text-[9px] uppercase tracking-widest"
                      icon={<Clock size={12} />}
                    >
                      Investigate
                    </Button>
                    <Button
                      ...
                      disabled={updating}
                      variant="secondary"
                      className="h-9 rounded-xl text-emerald-400 border-emerald-500/20 hover:bg-emerald-500 hover:text-white font-black text-[9px] uppercase tracking-widest"
                  ...
                  {/* Reply box */}
                  <div className="space-y-2">
                    <div className="rounded-xl border border-white/8 focus-within:border-rose-500/30 bg-white/[0.02] transition-colors">
                      <textarea
                        value={reply}
              ...
              {selected.status === "resolved" && (
                <div className="flex items-center justify-center gap-2 py-4 text-emerald-400 text-sm font-black uppercase tracking-widest border border-emerald-500/20 rounded-xl bg-emerald-500/5">
                  <CheckCircle2 size={18} />Complaint Resolved
                </div>
              )}
            </Card>
          ) : (
            <div className="h-full min-h-[400px] flex flex-col items-center justify-center gap-4 rounded-2xl border border-dashed border-white/10 opacity-40">
              ...
              <p className="text-sm font-black uppercase tracking-widest">Select a complaint to review</p>
            </div>
          )}
        </div>
      </div>
